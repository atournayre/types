...
class IbanTest extends \PHPUnit\Framework\TestCase
{
    public function testIBANValid()
    {
        ...
        foreach ($validIBANs as $iban) {
            try {
                $cleanIban = str_replace(' ', '', $iban[0]);
                Iban::fromString($cleanIban);
    ...
    public function testIBANInvalid()
    {
        ...
        foreach ($invalidIBANs as $iban) {
            ...
            $this->expectExceptionMessage(sprintf('Expected a value to contain letters and digits only. Got: "%s"', $iban[0]));
            Iban::fromString($iban[0]);
        }
    }
}
